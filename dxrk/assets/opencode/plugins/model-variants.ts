/**
 * model-variants
 * Exports per-model variant (effort level) data for dxrk.
 *
 ...
 * ~/.dxrk/cache/model-variants.json. dxrk reads this file
...
export const ModelVariantsPlugin: Plugin = async (input) => {
  async function refreshVariantsCache() {
    try {
      ...
      const cacheDir = path.join(homedir(), ".dxrk", "cache")
      await mkdir(cacheDir, { recursive: true })

      // Atomic write: write to .tmp then rename. rename() is atomic on POSIX,
      // so concurrent readers (e.g. `dxrk-py sync`) never see a partial JSON.
