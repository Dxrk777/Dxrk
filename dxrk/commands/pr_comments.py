...
def register_pr_comments_command(reg: Registry) -> None:
    ...
    def comments_run(ctx: CommandContext) -> int:
        ...
        if git_current_branch(wd) is None:
            ctx.err.write("Error: no se encontró PR para la rama actual\n")
            return 1
        ...
        out.write(f"Comentarios del PR #{number}:\n\n")
        out.write("## Comentarios generales\n")
        if not issue.ok or not issue.out.strip() or json.loads(issue.out or "[]") == []:
            out.write("No se encontraron comentarios.\n")
        else:
            try:
                for c in json.loads(issue.out):
                    out.write(f"- {c.get('user', {}).get('login', 'desconocido')}: {c.get('body', '')}\n")
            except json.JSONDecodeError:
                ctx.err.write(f"advertencia: no se pudieron obtener los comentarios del PR: {issue.err.strip()}\n")
        ...
        if not review.ok or not review.out.strip() or json.loads(review.out or "[]") == []:
            ...
            try:
                for c in json.loads(review.out):
                    ...
                    out.write(
                        f"- {c.get('user', {}).get('login', 'desconocido')} ({path}:{line}): {c.get('body', '')}\n"
                    )
            except json.JSONDecodeError:
                ctx.err.write(
                    f"advertencia: no se pudieron obtener los comentarios de revisión: {review.err.strip()}\n"
                )
        return 0
    ...
    def resolve_run(ctx: CommandContext) -> int:
        ...
        review = run_gh(wd, "api", f"repos/{{owner}}/{{repo}}/pulls/{number}/comments")
        if not review.ok:
            out.write("No hay hilos de comentarios para resolver.\n")
            return 0
        try:
            comments = json.loads(review.out)
        except json.JSONDecodeError:
            ctx.err.write(f"advertencia: no se pudieron obtener los comentarios de revisión: {review.err.strip()}\n")
            return 0
        ...
        for thread_id in threads:
            mutation = 'mutation { resolveReviewThread(input: {threadId: "' + str(thread_id) + '"}) { thread { id } } }'
            result = run_gh(wd, "api", "graphql", "-f", f"query={mutation}")
            if not result.ok:
                ctx.err.write(f"advertencia: no se pudo resolver el hilo {thread_id}: {result.err.strip()}\n")
                continue
            resolved += 1
        out.write(f"se resolvieron {resolved} hilo(s) de comentarios\n")
        return 0

    comments_cmd = Command(
        name="pr comments",
        short="Mostrar los comentarios del PR actual",
        run=comments_run,
    )
    resolve_cmd = Command(
        name="pr resolve",
        short="Resolver los hilos de revisión del PR actual",
        run=resolve_run,
    )
