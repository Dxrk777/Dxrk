...
class TestGoArch:
    def test_amd64(self):
        with patch("platform.machine") as m:
            m.return_value = "x86_64"
...
class TestRenderCli:
    ...
    def test_dev_build(self):
        r = UpdateResult(tool=ToolInfo(name="DXRK_MEMORY"), status=UpdateStatus.DEV_BUILD)
        output = render_cli([r])
