...
> **Baseline v0.1.2 (R06):** tabla estimada + cómo reproducir en [`benchmarks.md`](benchmarks.md) (detalle de cada caso en `benchmarks/README.md`) — `uv run python benchmarks/bench_memory.py --quick` (stdlib-only) y `uv run python -m pytest benchmarks/ -q` (fallback sin `pytest-benchmark`).
